...
Optimisations :
- Appels API ENTSO-E parallélisés (ThreadPoolExecutor)
- make_subplots : shapes hline construites en une passe (pas de add_hline en boucle)
- Cache charger_periode avec ttl=1800 pour éviter les données périmées
...
MAX_WORKERS_API = 4  # parallélisme API
...
def ajouter_au_parquet(df_nouveau: pd.DataFrame) -> None:
    ...
    df_final = df_nouveau if df_existant.empty else pd.concat([df_existant, df_nouveau], axis=0)
...
# ═══════════════════════════════════════════════════════════════════
# 2. FONCTIONS API ENTSO-E
# ═══════════════════════════════════════════════════════════════════

def api_telecharger_jour(jour_str: str) -> pd.DataFrame:
    """Télécharge un jour depuis ENTSO-E."""
...
st.caption(
    ...
    "Source données : parquet local + ENTSO-E API pour les jours manquants"
)

with st.sidebar:
    st.header("📅 Période")
    st.caption(
        "ℹ️ Les données sont lues depuis le fichier local. "
        "Les jours manquants sont téléchargés en parallèle."
    )
    ...
    st.markdown("---")
    df_parquet_info = lire_parquet()
    if df_parquet_info.empty:
        st.caption("📂 Aucun fichier parquet local trouvé.")
        st.caption(f"🔍 Chemin cherché :\n`{FICHIER_PARQUET}`")
    else:
        jours_dispo = jours_dans_parquet(df_parquet_info)
        st.caption(
            f"💾 Parquet local : **{len(jours_dispo)} jours** en cache\n\n"
...
# Vider le cache uniquement sur clic explicite du bouton Rafraîchir,
# pas sur le chargement automatique initial
if lancer and not premiere_visite:
    st.cache_data.clear()  # vide lire_parquet + charger_periode en une fois
...
# ═══════════════════════════════════════════════════════════════════
# 4. CHARGEMENT — OPTIMISATION 1 : API parallèle + OPTIMISATION 3 : ttl cache
# ═══════════════════════════════════════════════════════════════════

@st.cache_data(ttl=1800, show_spinner=False)  # ← FIX 3 : ttl pour éviter données périmées
def charger_periode(start_str: str, end_str: str) -> pd.DataFrame:
    """
    - Jours présents dans le parquet → lecture locale instantanée
    - Jours manquants → fetch ENTSO-E en parallèle (ThreadPoolExecutor)
    """
    ...
    # Extraire depuis le parquet
    if not df_parquet.empty and len(jours_a_fetcher) < nb_jours:
        ...
        morceaux_parquet.append(df_parquet.loc[masque])

    # ── FIX 1 : téléchargement parallèle ──────────────────────────
    nouveaux_morceaux = []
    echecs = []

    if jours_a_fetcher:
        barre = st.progress(0.0, text="⚡ Téléchargement parallèle des jours manquants…")
        compteur = {"n": 0}

        def fetch_et_sauvegarder(j):
            df_j = api_telecharger_jour(str(j))
            if df_j is not None and not df_j.empty:
                ajouter_au_parquet(df_j)
            return j, df_j

        with ThreadPoolExecutor(max_workers=MAX_WORKERS_API) as executor:
            futures = {executor.submit(fetch_et_sauvegarder, j): j for j in jours_a_fetcher}
            for future in as_completed(futures):
                try:
                    j, df_j = future.result()
                    if df_j is not None and not df_j.empty:
                        nouveaux_morceaux.append(df_j)
                except Exception as e:
                    echecs.append((str(futures[future]), str(e)))
                finally:
                    compteur["n"] += 1
                    barre.progress(
                        compteur["n"] / len(jours_a_fetcher),
                        text=f"⚡ Téléchargé {compteur['n']}/{len(jours_a_fetcher)} jours…"
                    )
    ...
    tous_morceaux = morceaux_parquet + nouveaux_morceaux
    if not tous_morceaux:
        return pd.DataFrame()

    df_total = pd.concat(tous_morceaux, axis=0)
...
df_parquet_apres  = lire_parquet()
jours_caches_apres = jours_dans_parquet(df_parquet_apres)
nb_depuis_parquet  = sum(1 for j in pd.date_range(start_date, end_date).date if j in jours_caches_apres and j < AUJOURDHUI)
nb_depuis_api      = nb_jours - nb_depuis_parquet

st.success(
    f"✅ Données chargées — {start_date} → {end_date} · "
    f"💾 {nb_depuis_parquet} jour(s) depuis le parquet · "
    f"🌐 {nb_depuis_api} jour(s) téléchargé(s) depuis l'API"
)
...
# ═══════════════════════════════════════════════════════════════════
# 8. SPARKLINES — OPTIMISATION 2 : shapes en une seule passe
# ═══════════════════════════════════════════════════════════════════
...
for idx, reacteur in enumerate(reacteurs):
    ...
    fig_spark.add_trace(go.Scatter(
        ...
        hovertemplate=(
            ...
            "%{x}<br>"
            "<b>%{customdata:.0f} MW</b> · %{y:.1f} % Pnom"
            "<extra></extra>"
        ),
    ), row=row, col=col)

# ── FIX 2 : toutes les lignes de référence à 100 % en une seule passe ──
shapes_hline = []
for idx in range(len(reacteurs)):
    row = idx // N_COLS_SPARKLINES + 1
    col = idx % N_COLS_SPARKLINES + 1
    # calcul du ref plotly pour chaque subplot
    ax_idx = idx + 1
...
fig_spark.update_layout(
    ...
    shapes=shapes_hline,  # ← une seule mise à jour au lieu de 56 add_hline
)
